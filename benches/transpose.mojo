...
from mograd.runtime.gpu.kernels.dispatch import UnaryStrided


def _transposed_view_layout(batch: Int, M: Int, N: Int) raises -> Layout:
    if batch == 1:
        return Layout(M, N).transpose()
    return Layout(batch, M, N).transpose()
...
def main() raises:
    ...
    comptime for di in range(len(dtypes)):
        ...
        comptime for si in range(len(sizes_2d_m)):
            comptime M = rebind[Int](sizes_2d_m[si])
            comptime N = rebind[Int](sizes_2d_n[si])
            comptime for ki in range(len(syms)):
                bench_transpose[dtype, 1, M, N](m, device=device, ctx=ctx, symbol=rebind[String](syms[ki]))

        comptime for si in range(len(sizes_3d_b)):
            comptime B = rebind[Int](sizes_3d_b[si])
            comptime M = rebind[Int](sizes_3d_m[si])
            comptime N = rebind[Int](sizes_3d_n[si])
            comptime for ki in range(len(syms)):
                bench_transpose[dtype, B, M, N](m, device=device, ctx=ctx, symbol=rebind[String](syms[ki]))
