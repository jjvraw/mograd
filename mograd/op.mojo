...
struct OpRef(Copyable, ImplicitlyCopyable, KeyElement, Movable, Writable):
    ...
    def cast(self, out_dtype: DType) -> OpRef:
        # Casting to the dtype the node already carries is decidable from the
        # arguments alone, so fold it here rather than leaving a no-op CAST for
        # the rewrites and gradient rules to step around.
